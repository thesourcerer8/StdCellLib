...
/*
 WARNING:
 This is a special purpose liberty file with a huge number of cells that represent any kind of combinatorial logic (with maximum $maxpins pins), to evaluate which logic functions are actually wanted by synthesizing tools. Afterwards the actually used cells are the ones that should be provided by the standard cell library. Let's try to think demand-driven.
...
foreach my $npins(1 .. $maxpins)
{
  my @inputs=();
 
  foreach my $mag(0 .. (2**$npins)-1)
 {
